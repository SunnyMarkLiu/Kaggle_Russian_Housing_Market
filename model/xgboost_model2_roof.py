...
train_predict = pd.DataFrame({'id': id_train,
                              'xgboost2_oof_predict': model.predict(dtrain)})
test_predict = pd.DataFrame({'id': id_test,
                             'xgboost2_oof_predict': model.predict(dtest)})

train_predict.to_csv(Configure.train_cv_result_for_model_stacking.format('xgboost_model2', time.strftime('%Y-%m-%d_%H:%M:%S',
                                                                                                  time.localtime(
                                                                                                      time.time()))),
                     index=False)
test_predict.to_csv(Configure.test_cv_result_for_model_stacking.format('xgboost_model2', time.strftime('%Y-%m-%d_%H:%M:%S',
                                                                                                time.localtime(
                                                                                                    time.time()))),
                    index=False)
