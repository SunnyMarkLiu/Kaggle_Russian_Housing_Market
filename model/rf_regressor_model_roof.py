...
def main():
    ...
    for i, (train_index, test_index) in enumerate(kfold.split(train)):
        ...
        model = RandomForestRegressor(n_jobs=20) #We have chosen to just normalize the data by default, you could GridsearchCV this is you wanted
        ...
        model = RandomForestRegressor(n_estimators=grid.best_estimator_.n_estimators,
                                      max_depth=grid.best_estimator_.max_depth, n_jobs=20)  # paramters tuned using GridSearchCV
        model.fit(x_tr, y_tr)
    ...
    train_predict = pd.DataFrame({'id': id_train,
                                  'random_forest_oof_predict': oof_train})
    test_predict = pd.DataFrame({'id': id_test,
                                 'random_forest_oof_predict': oof_test})

    train_predict.to_csv(Configure.train_cv_result_for_model_stacking.format('random_forest', time.strftime('%Y-%m-%d_%H:%M:%S', time.localtime(time.time()))),
                         index=False)
    test_predict.to_csv(Configure.test_cv_result_for_model_stacking.format('random_forest', time.strftime('%Y-%m-%d_%H:%M:%S', time.localtime(time.time()))),
                         index=False)
