...
def main():
    ...
    train_predict = pd.DataFrame({'id': id_train,
                                  'extratrees_oof_predict': oof_train})
    test_predict = pd.DataFrame({'id': id_test,
                                 'extratrees_oof_predict': oof_test})

    train_predict.to_csv(Configure.train_cv_result_for_model_stacking.format('extratrees', time.strftime('%Y-%m-%d_%H:%M:%S', time.localtime(time.time()))),
                         index=False)
    test_predict.to_csv(Configure.test_cv_result_for_model_stacking.format('extratrees', time.strftime('%Y-%m-%d_%H:%M:%S', time.localtime(time.time()))),
                         index=False)
